...
def petal_path(length, width, tip_round, base_round=6):
    return (
        f"M 0,0 "
        f"C {-width:.2f},{-length*0.35:.2f} {-tip_round:.2f},{-length:.2f} 0,{-length:.2f} "
        f"C {tip_round:.2f},{-length:.2f} {width:.2f},{-length*0.35:.2f} 0,0 "
        f"Z"
    )
...
light_path = petal_path(length=128, width=30, tip_round=11)
dark_path = petal_path(length=76, width=38, tip_round=17)
...
outer_r = 156
leaf_angle_deg = 128  # from top, clockwise
...
leaf_tilt = 12  # slight tilt, tip dips down/out
...
out = "/tmp/claude-0/-home-user-logo-gen-test/66515361-52e5-52b5-a0b3-4f402867e455/scratchpad/logo/logo.svg"
